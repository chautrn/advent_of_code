	...
	for _, draw := range draws {
		for boardIndex, board := range boards {
			...
			if evaluateBoard(&board) {
				contains := false
				for _, p := range boardsWon {
					if p.boardIndex == boardIndex {
						contains = true
					}
				}
				if !contains {
					boardsWon = append(boardsWon, bdPair{boardIndex, sumUnmarked(&board) * draw})
				}
			}
		}
	}
